    ...
    for bar_step in range(0, global_end, BAR_RESOL):
        ...
        for timing in range(bar_step, bar_step + BAR_RESOL, TICK_RESOL):
            ...
            if len(t_tempos) or len(t_chords):
                ...
                # create
                pos_events.append(create_piano_metrical_event(tempo_text, chord_text, pos_text))
                pos_on = True

            # note
            if len(t_notes):
                if not pos_on:
                    pos_events.append(create_piano_metrical_event("CONTI", "CONTI", pos_text))
                ...
                for note in t_notes:
                    ...
                    pos_events.append(create_piano_note_event(note_pitch_text, note_duration_text, note_velocity_text))
