...
class MemTransformerLM(nn.Module):
    def __init__(self, modelConfig, n_token, tie_projs=[False], cutoffs=[], is_training=True):
    ...
    def _forward(self, dec_inp, mems=None):
        ...
        if self.same_length:
            ...
            if mask_len > 0:
                ...
                mask_shift_len = qlen
            dec_attn_mask = (torch.triu(all_ones, 1 + mlen) + torch.tril(all_ones, -mask_shift_len)).bool()[:, :, None]  # -1
        else:
            dec_attn_mask = torch.triu(word_emb.new_ones(qlen, klen), diagonal=1 + mlen).bool()[:, :, None]
